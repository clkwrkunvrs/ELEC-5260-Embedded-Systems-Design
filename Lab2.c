...
static uint8_t count = 0; /* determines where in the count the program is */
/*---------------------------------------------------*/
...
void PinSetup () {
 ...
 GPIOC->MODER &= ~(0x0000FF); /*Clear PC[3:0] Mode bits*/
...
void counting(char direction) {
        ...
        if((direction == 0x04) & (count == 0x0)) count = 9;
        else if((direction == 0x0) && (count == 9)) count = 0;
        else direction ? count -- : count++;
                }
/*----------------------------------------------------------*/
 ...
 while (1) { //Can also use: for(;;) {
         ...
         while(sw1) {
                  ...
                  sw2 = GPIOA->IDR & 0x04; //Read GPIOA and mask all but bit 2 (PA2)
                        ...
                        GPIOC->ODR = (uint16_t)count & 0x000F; //AND the count bits with 0x111F to get binary value of c
ount
                        //alternative could be a loop with (count >> k) & 1. Also, don't care about upper ODR bits for t
his project
